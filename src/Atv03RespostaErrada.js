...
app.get('/cadastro', (req,res) => {
  ...
  const controle = req.query.controle
  const personagem = req.query.personagem

  if(personagem.length > 5){
    res.send("Quantidade de personagens escolhida superior ao necessário (5)")
  }
  else if(personagem.length < 5){
    res.send("Quantidade de personagens escolhida inferior ao necessário (5)")
  }else{
